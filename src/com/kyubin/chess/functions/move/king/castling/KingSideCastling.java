...
public class KingSideCastling {
    public static boolean isPossibleKingSideCastling(boolean is_white, ChessGame chessGame, Piece king){
        ...
        if(is_white){
            if(!PieceFunctions.isPieceGoThere(new XY(6,8),false,chessGame).isEmpty()) return false;
            return PieceFunctions.isPieceGoThere(new XY(7, 8), false, chessGame).isEmpty();
        } else {
            if(!PieceFunctions.isPieceGoThere(new XY(6,1),true,chessGame).isEmpty()) return false;
            return PieceFunctions.isPieceGoThere(new XY(7, 1), true, chessGame).isEmpty();
        }
    }
}
